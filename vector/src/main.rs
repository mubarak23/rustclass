...
fn main() {
    ...
    // vec!(..) and vec![..] are same macros, so the are both the same
    let v: Vec<u8> = vec!(1, 2, 3);
    is_vec(v.clone());
...
// The capacity of a vector is the amount of space allocated for any future elements that will be added onto the vector.
//  This is not to be confused with the length of a vector, which specifies the number of actual elements within 
//  the vector. If a vector’s length exceeds its capacity, 
// its capacity will automatically be increased, but its elements will have to be reallocated. 
// // FIX the errors
fn main() {
